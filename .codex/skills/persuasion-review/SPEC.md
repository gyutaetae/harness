...
현재 서비스(또는 신규 기능)에 대한 가치제안이 **실제 잠재고객 의사결정 네트워크에서 얼마나 설득력을 갖는지**를, 고객 페르소나 기반 다중 codex exec headless 세션으로 시뮬레이팅하여 비판적으로 검토한다. 결과를 누적해 향후 실제 고객 피드백과 비교·보정(캘리브레이션)할 기반을 만든다.
...
```
<repo_root>/
├── .codex/skills/persuasion-review/    # skill 정의 (Codex가 읽음)
│   ├── SKILL.md
│   ├── SPEC.md
│   └── scripts/
│       ├── run_simulation.py           # 메인 오케스트레이터
│       ├── session_runner.py           # codex exec 래퍼
│       └── prompts/                    # 단계별 system prompt 템플릿
│           ├── keyman_initial.md
│           ├── stakeholder_review.md
│           ├── keyman_response.md
│           └── final_analyzer.md
└── persuasion-data/                    # 데이터 (Write 대상 — .codex/ 밖에 둠)
    ├── personas/
    │   └── <persona_id>/
...
**.codex/ 밖에 데이터를 두는 이유**: nested Codex 세션이 skill bundle 자체를 수정하지 않도록 실행 데이터와 skill 정의를 분리한다. personas(익명 설계)·feature-ideas는 commit 대상, runs는 per-machine 로컬 출력.
...
모든 codex exec headless 세션의 출력 파일은 아래 frontmatter를 반드시 포함 (system prompt로 강제).
...
2. 사용자가 덩어리 텍스트 입력.
3. Codex가 스키마에 채운 draft markdown을 생성하여 보여줌. 채우지 못한 필드는 `unknown` 명시.
...
3. **가치제안 문서 작성**: 선택된 전달 모드에 맞춰 Codex가 draft 작성 → 사용자와 티키타카로 컨펌 → `value_proposition.md` 저장.
...
`scripts/run_simulation.py`가 아래 단계를 오케스트레이션. **각 단계는 codex exec headless subprocess로 실행**되며, 지정 경로에 지정 포맷(frontmatter 포함)으로 결과 파일을 남기도록 system prompt로 강제.
...
#### 3.C.3 단계 (Codex 대화, 최종 리포트)

- `final_analyzer.md` system prompt와 함께 모든 run 파일을 읽고 Codex가 분석:
  - **최종 판정**: 계약 성사 / 실패
...
### 4.1 codex exec headless 호출 템플릿

```bash
codex exec --skip-git-repo-check --dangerously-bypass-approvals-and-sandbox "$(cat <<EOF
{system_prompt}
...
Codex CLI에서는 현재 `--dangerously-skip-permissions`가 아니라 `--dangerously-bypass-approvals-and-sandbox`가 대응 플래그다. 각 세션 prompt는 **자신의 output 파일 쓰기 + 주어진 입력 파일 읽기**만 허용하도록 제한한다.
