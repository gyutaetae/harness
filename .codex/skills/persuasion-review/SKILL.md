...
description: 잠재고객 페르소나 기반으로 현재 서비스/신규 기능의 설득력을 다중 codex exec headless 세션으로 비판적 시뮬레이팅. 트리거 — "현재 서비스의 설득력을 검토하고싶어", "설득력 검토", "고객 시뮬", "시뮬 돌려줘", "새 고객 프로파일 만들자", "페르소나 만들자", "고객 프로파일 수정하자", "고객 프로파일 보여줘", "페르소나 목록" 및 유사 의도.
---
...
5. **경로**. skill 정의(SKILL.md / SPEC.md / scripts/)는 `.codex/skills/persuasion-review/`에, 데이터(`personas/`, `runs/`, `feature-ideas.md`)는 레포 루트 `persuasion-data/`에 둔다. nested Codex 세션이 skill bundle을 직접 수정하지 않도록 실행 데이터는 `.codex/` 밖에 둔다. 아래 언급되는 `personas/` · `runs/` 경로는 모두 `persuasion-data/` 기준.
...
- 전달 모드에 맞춰 Codex가 draft 작성. 랜딩 카피, 기능 설명, 가격 플랜 등.
...
```bash
# repo root에서 실행
uv run --with pyyaml python .codex/skills/persuasion-review/scripts/run_simulation.py \
...
`codex` CLI는 시스템에 설치되어 있다고 가정. nested 세션은 기본적으로 Codex의 `--dangerously-bypass-approvals-and-sandbox` 플래그를 사용한다.
